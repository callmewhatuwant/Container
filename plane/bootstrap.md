...
change script for podman compatibility
...
function buildLocalImage() {
    ...
    then
        ...
        podman-compose - build.yml build --no-cache  >&2
        # cd $CURR_DIR
...
function download() {
    ...
    if [ $USE_GLOBAL_IMAGES == 0 ]; then
        ...
        podman-compose -f $DOCKER_FILE_PATH --env-file=$DOCKER_ENV_PATH pull
    fi
...
function startServices() {
    podman-compose -f $DOCKER_FILE_PATH --env-file=$DOCKER_ENV_PATH up -d --quiet-pull
...
function stopServices() {
    podman-compose -f $DOCKER_FILE_PATH --env-file=$DOCKER_ENV_PATH down
}
...
function viewSpecificLogs(){
    ...
    if podman-compose -f $DOCKER_FILE_PATH ps | grep -q "$SERVICE_NAME"; then
        echo "Service '$SERVICE_NAME' is running."
    else
        echo "Service '$SERVICE_NAME' is not running."
    fi

    podman-compose -f $DOCKER_FILE_PATH logs -f $SERVICE_NAME
}
function viewLogs(){
...
change yaml for podman compatibility
...
change plane.env default nginx port from 80 to 8080 because 80 is in use
...
select 2
info:
ctrl c when api start takes forever, everything will start dont panic

add email invite
go to yourdomain/god-mode/email

settigns for gmail
first add 2FA then  add app password in google acc
